    ...
    if (myfile.is_open()) // Check if file to format is actually open.
    {
        ...
        {   
                                            ...
                                            // the given threshold, program keeps chopping them into more lines.
            {
                ...
                while(!std::isspace(line.at(break_point)) && break_point < line.length()-1) // If the max_char-th character is not a white space,
                {                                                                           // program must find the nearest white space after the 
                    ++break_point;                                                          // max_char-th character, BEFORE the end of the line.
                }                                                                           //
                myfile_formatted << line.substr(0,break_point+1) << '\n'; // Once it finds it, the line is chopped into
                                                                          // two lines: the first from the beginning of
                                                                          // the original line to the break_point just 
                                                                          // found.
                line = line.substr(break_point+1,line.length()-break_point);   // The second from the break_point+1-th char (from after white space)
                                                                               // to the end of the line: this becomes the new line which length 
                                                                               // is checked by the first while loop.
                                                                               // 
            }
            if(line != "") myfile_formatted << line << '\n'; // The last piece of line, which is shorter than max_char, is ignored by the 
                                                             // first while loop. Thus, it must be printed on file separately, if it is not
                                                             // an empty line.
        }
